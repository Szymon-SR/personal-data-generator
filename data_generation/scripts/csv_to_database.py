...
def run():
    try:
        with open(CSV_FILENAME) as file:
            ...
            for row in reader:
                if not str(row[1]).startswith('Skrytki') and not str(row[1]).startswith('Instytucja'):
                    print(row)

                    new_address = PostAddress(city=row[2], post_code=row[0], voivodeship=row[3], county=row[4])
                    new_address.save()
